%% This is for Mohsen's Mg4Al sample

%load data
close all;
clc;
sample_name = 'MgAl';
working_dir = 'E:\zhec umich Drive\2021-10-01 MgAl insitu curve';
cd(working_dir);
...
width = 3.00;       % UM134_Mg_D3, tested on 2021-09-11
thickness = 2.3;
...
xlabel('time (sec)');
ylabel('displacement (mm)');
set(gca,'fontsize',12,'xlim',[0 1500]);
...
ylabel('Stress (MPa)');
set(gca, 'xlim',[-0.005, 0.03], 'ylim',[-20 200], 'fontsize',18);
...
ylabel('Stress (MPa)');
set(gca, 'xlim',[-0.1, 1.1], 'ylim',[-20 200], 'fontsize',18);
...
for iE = 0:3
    ...
    plot(strain(ind_stop(1:4)), stress(ind_stop(1:4)), 'r.', 'markersize', 18);
