%% Note: in this sample, there are many twins due to polishing. They are currently considered as individual grains. 
%% setup
clear; clc; close all;
addChenFunction;
...
set(gca,'fontsize',18);
label_map_with_ID(x,y,ID_0,gcf, [49,46,338,312],'r',24,10);  % illustrate selected control grains  
...
ID_list{1} = [15,15,17,160];
tolerance_cell{1} = [3,3,3,3];
...
strain_ebsd = [0.0000, -0.0288, -0.0378, -0.0454, ...
    -0.0405, -0.0308, -0.0144, -0.0012, ...
    -0.0177, -0.0315, -0.0428, ...
    -0.0296, -0.0144, -0.0016];
...
set(gca,'xlim',[-0.035, 0.005],'ylim',[-2 40],'fontsize',18,'fontweight','normal');
...
set(gca,'xdir','normal','linewidth',1.5);
set(gca,'xlim',[-0.055, 0.005],'ylim',[-2 40],'fontsize',18,'fontweight','normal');
...
set(gca,'xdir','normal','linewidth',1.5);
set(gca,'xlim',[-0.065, 0.005],'ylim',[-2 40],'fontsize',18,'fontweight','normal');
xlabel('Strain from ebsd estimate');
ylabel('Twin Area Percent (%)');
