...
try
    load(fullfile(save_dir,['Mg4Al_C3_mask_cell.mat']),'mask_cell');
catch
   mask_cell = cell(1,13); 
end
...
% initial_number_of_grains = length(unique(ID(:)));
% In the next part, to check if the added boundary segment just create one
% addtional grain, we use function find_ID_map_from_boundary_map() to
% check the result number of unique grains. Therefore, here, the reference
% should also be generated by the same function.  The ID map find by this
% function can be a little bit different from that by OIM analysis, but
% it's hard to make the result exactly the same using my custom code...
ID_ = find_ID_map_from_boundary_map(boundary);   
initial_number_of_grains = length(unique(ID_(:)));
str = sprintf('iE = %d, initial number of grains: %d', iE, initial_number_of_grains);
disp(str);
...
while iN <= N
    ...
    % Then find temp id map from boundary. Ideally, each mask should create
    % one more grain. Otherwise, do not record mask, do not increment iN,
    % and do it again
    ID_temp = find_ID_map_from_boundary_map(boundary_new);
    current_number_of_grains = length(unique(ID_temp(:)));
    % smaller than, to remove gb
    if (current_number_of_grains - initial_number_of_grains == iN) 
        str = sprintf('  now number of grains on map: %d', current_number_of_grains);
        disp(str);
        iN = iN + 1;
    else
        mask_cell{iE}{iN} = [];     % delete mask
        boundary_new(indR_min:indR_max, indC_min:indC_max) = boundary_local;    % change bandary_new back to before modified
        str = sprintf('  now number of grains on map: %d', current_number_of_grains);
        disp(str);
        warning(' More than one additional number of grains is created, you might want to do it again!');
    end
    
    save(fullfile(save_dir,['Mg4Al_C3_mask_cell.mat']),'mask_cell');
end
