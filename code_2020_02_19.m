%% manual mode: find relative position for sample UM-134 A2 after EBSD images
if 0
    addChenFunction;
    cd('D:\UMich Folder\11 ONR project\32 Microscopy\2020-02-07 UM-134 A2');
    a = imread('ref whole area actual_B_001.tif');
    a = imcrop(a,[0 0 6144 4096]);
    b = imread('ref whole area actual_B_002.tif');
    b = imcrop(b,[0 0 6144 4096]);
    c = imread('ref whole area actual_B_003.tif');
    c = imcrop(c,[0 0 6144 4096]);
    d = imread('ref whole area actual_B_004.tif');
    d = imcrop(d,[0 0 6144 4096]);
    
    %%
    [aby,abx] = normxcorr2A_register(b,a,[0 0 2000, 0],[0 0 0 0],1)
    [acy,acx] = normxcorr2A_register(c,a,[0 2000, 0, 0], [2000, 0, 0, 0],1)
    [cdy,cdx] = normxcorr2A_register(d,c,[0 0 2000, 0],[0 0 0 0],1)
    [bdy,bdx] = normxcorr2A_register(d,b,[0 2000, 0, 0], [2000, 0, 0, 0],1)
    ady = (acy + cdy + aby + bdy)/2
    adx = (acx + cdx + abx + bdx)/2
end

%% modify from m_4_findImagePositionAndStitch
addChenFunction;
...
transX_incremental = zeros(row_end,col_end);
transY_incremental = zeros(row_end,col_end);
transX = zeros(row_end,col_end);
transY = zeros(row_end,col_end);
...
for iR = row_start:row_end
    for iC = col_start:col_end
        imgName = [fileNamePrefix,'r',num2str(iR),'c',num2str(iC),'.tif']
        I = imread(imgName);
        I = imcrop(I,[0 0 resX, resY]);
        imwrite(I, imgName, 'tif');
    end
end
...
for iC = col_start:col_end
    for iR = row_start:row_end - 1 + singleRow
        ...
        close all;
        try
            fName1 = [fileNamePrefix,'r',num2str(iR),'c',num2str(iC)]; disp(fName1);
            fName2 = [fileNamePrefix,'r',num2str(iR+1),'c',num2str(iC)]; disp(fName2);
            I = imread([path_img,'\',fName1,'.tif']);
            J = imread([path_img,'\',fName2,'.tif']);
            ...
            try
                switch corrMethod
                    ...
                    case 3
                        [yOffSet,xOffSet] = normxcorr2A_register(J, I, [0, 0.7*size(J,1),  0, 0], [0.7*size(I,1), 0,  0, 0], 1)
                end
        ...
        if (iR==row_start)&&(iC<col_end)    % search the right-side picture as well
            ...
            close all;
            fName1 = [fileNamePrefix,'r',num2str(iR),'c',num2str(iC)]; disp(fName1);
            fName3 = [fileNamePrefix,'r',num2str(iR),'c',num2str(iC+1)]; disp(fName3);
            I = imread([path_img,'\',fName1,'.tif']);
            J = imread([path_img,'\',fName3,'.tif']);
            ...
            try
                switch corrMethod
                    ...
                    case 2
                        ...
                        yOffSet = size(I,1) - osy_neg;
                    case 3
                        [yOffSet,xOffSet] = normxcorr2A_register(J, I, [0, 0,  0, 0.7*size(J,2)], [0, 0,  0.7*size(I,2), 0], 1);
...
save([path_img,'\','translations_searched_vertical'],'transX','transY','transX_incremental','transY_incremental');
...
for iR = row_start:row_end
    for iC = col_start:col_end
        fName = [fileNamePrefix,'r',num2str(iR),'c',num2str(iC)]; disp(fName);    % change this accordingly ----------------------------------------------------------
        
        I = double(imread([path_img,'\',fName,'.tif']));    % otherwise, nanmean doesn't work, bucause cat makes nan to 0.
...
if bs > (2^32-1)*4
    img = uint8(img/255);
    imwrite(img,[path_img,'\img_full_res.tif'],'tif');
    %     imwrite(img(1:2:end,1:2:end),[path_img,'\img_half_res.tif'],'tif');
    %     imwrite(img(1:10:end,1:10:end),[path_img,'\img_10_res.tif'],'tif');
